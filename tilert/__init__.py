...
# Map model_type -> backend shared library. Both register the "tilert" torch
# op namespace, so exactly one may be loaded per process.
_BACKENDS = {
...
def load_backend(model_type: str) -> None:
    """Load the C++ backend for ``model_type`` (lazy, once per process).
