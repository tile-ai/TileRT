...
def merge_per_step(per_step_list: list[PerStepDict]) -> PerStepDict:
    """Merge multiple per-step dicts by mode label (same pattern as merge_stats)."""
    merged: PerStepDict = {}
    for ps in per_step_list:
        for mode, cols in ps.items():
            mode_dict = merged.setdefault(mode, {})
            for col, data_list in cols.items():
                mode_dict.setdefault(col, []).extend(data_list)
    return merged
