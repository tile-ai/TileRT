...
KI_PAGE_BYTES = (PAGE_SIZE * INDEX_HEAD_DIM
                 + PAGE_SIZE * INDEX_HEAD_DIM // KI_QUANT_BLOCK * 4)  # 8448
...
def _hadamard(x: torch.Tensor) -> torch.Tensor:
    """Hadamard rotation of the last dim (scale d^-0.5), matching TileRT's
    indexer. fast_hadamard_transform if present, else a scipy matmul."""
    d = x.shape[-1]
    try:
...
class MlaNsaProfile:
    """Config-driven MLA+NSA profile. ``engine_factory(weights, max_seq,
    with_mtp, ar_steps) -> adapter`` builds the model-specific engine."""

    num_ranks = wire.NUM_RANKS
    sender_ranks = frozenset({0})   # MLA latent replicated across TP

    def __init__(self, name: str, num_layers: int, layout_version: int,
                 engine_factory, mla_fp8: bool = True):
        self.name = name
    ...
    def buffer_bytes(self, max_seq_len: int) -> int:
        return (self._kv_plane(max_seq_len) + self._pe_plane(max_seq_len)
                + self._ki_plane(max_seq_len))
    ...
    def convert(self, buffer, base_ptr, max_seq_len, received, num_devices=1):
        ...
        for lid in range(self.num_layers):
            ...
            if self.mla_fp8:
                ...
                kv = kv_raw.view(torch.bfloat16).view(seq, KV_LORA_RANK) \
                    .contiguous()                       # already bf16
    ...
    def _dequant_kv(kv_raw: torch.Tensor, seq_len: int) -> torch.Tensor:
        ...
        fp8 = kv_raw[:, :KV_FP8_BYTES].reshape(-1).view(torch.float8_e4m3fn) \
            .reshape(seq_len, KV_LORA_RANK)
        scale = kv_raw[:, KV_FP8_BYTES:].reshape(-1).contiguous() \
            .view(torch.float32).reshape(seq_len, nblk)
    ...
    def _dequant_ki(ki_raw: torch.Tensor, seq_len: int) -> torch.Tensor:
        ...
        ki_fp8 = ki_raw[:, :fp8_bytes].reshape(-1).view(torch.float8_e4m3fn) \
            .reshape(npages * PAGE_SIZE, INDEX_HEAD_DIM)
        scale = ki_raw[:, fp8_bytes:].reshape(-1).contiguous() \
            .view(torch.float32).reshape(npages * PAGE_SIZE,
                                         INDEX_HEAD_DIM // KI_QUANT_BLOCK)
    ...
    def classify_layers(self, kv_caches: dict, kv_cache_config) -> _Reg:
        group_of = {}
        for gi, g in enumerate(getattr(kv_cache_config, "kv_cache_groups",
                                       []) or []):
        ...
        logger.info("%s registered %d MLA + %d KI layers, MLA cache=%s",
                    self.name, len(mla), len(ki),
    ...
    def extract(self, reg: _Reg, m, tp_rank, staging, max_seq_len):
        ...
        slots = (offs.reshape(1, -1)
                 + bt.reshape(-1, 1) * PAGE_SIZE).flatten()[:seq]
    ...
    def rdma_plan(self, hello, sections, tp_rank, seq_len, base):
        ...
        kv_bpt = self._kv_bpt
        r_kv, r_pe, r_ki = (int(hello["kv_base"]), int(hello["pe_base"]),
                            int(hello["ki_base"]))
        for lid in range(self.num_layers):
    ...
    def build_engine(self, model_weights_dir, max_seq_len, with_mtp, ar_steps):
        return self._engine_factory(model_weights_dir, max_seq_len, with_mtp,
                                    ar_steps)
...
class MlaNsaEngineAdapter:
    ...
    def __init__(self, generator, with_mtp: bool):
        ...
        self.max_seq_len = getattr(generator.decode_layer, "max_seq_len",
                                   200000)
    ...
    def decode(self, first_token_id, max_tokens, sampling, on_token=None,
        ...
        if temp < 1e-5:
            self.gen.update_sampling_params(
                temperature=1.0, top_p=1.0, top_k=1, use_topp=False)
        else:
            self.gen.update_sampling_params(
                temperature=temp, top_p=float(sampling.get("top_p", 0.95)),
                top_k=int(sampling.get("top_k", 256)), use_topp=True)
        budget = min(int(max_tokens), self.max_seq_len - self._seq_len - 1)
        if budget <= 0:
            self.last_stats = {"finish_reason": "length"}
            return [int(first_token_id)]
        if self.with_mtp:
            return self._decode_mtp(first_token_id, budget, on_token,
                                    cancel_event)
        return self._decode_standard(first_token_id, budget, on_token,
                                     cancel_event)
    ...
    def _decode_mtp(self, first_token_id, budget, on_token, cancel_event):
        ...
        draft = torch.full((1, T), int(self._last_prompt_token),
        ...
        while not finished and len(tokens) < budget:
            if cancel_event is not None and cancel_event.is_set():
                ...
                break
            if fwd == 1:
                draft = torch.full((1, T), int(first_token_id),
                                   dtype=torch.int32, device="cuda:0")
            elif fwd > 1:
    ...
    def _decode_standard(self, first_token_id, budget, on_token, cancel_event):
        ...
        if int(first_token_id) in stop_ids:
            ...
            return []
        finish = "length"
        cur = torch.tensor(int(first_token_id), dtype=torch.long,
                           device="cuda:0")
