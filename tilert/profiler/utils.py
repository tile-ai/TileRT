...
class WorkerBookVisualizer:
    ...
    def add_region_cell_by_time(
        ...
        row_span = max(op_end_row_idx - op_start_row_idx, 1)

        if row_span <= 2 and short_label:
            display_text = short_label
        elif row_span <= 4 and short_label:
            dur_us = (end_time - start_time) / 1000
            display_text = f"{short_label}\n{dur_us:.2f}us"
        else:
            display_text = op_show_info
    ...
    def timeline_visual_region(
        ...
        row_height = self.style_configs.row_height_pt
        total_rows = int(np.ceil(total_end_time / ns_tick)) + 2
    ...
    def add_sm_brief_sheet(self, profile_logs: np.ndarray, sheet_name: str) -> WorkerBookVisualizer:
...
def visualize_profile_logs(
    ...
    visualizer.add_sheet(assemble_profile_logs, "timeline")

    if with_mean:
        visualizer.add_sheet(mean_profile_logs, "mean")

    visualizer.add_sm_brief_sheet(filtered_logs, "sm_brief")
...
def parse_profile_log_tensor(
    ...
    if inst2opname is not None:
        profile_logs = profile_logs[:, : len(inst2opname), :]
