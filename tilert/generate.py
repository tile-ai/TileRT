...
if __name__ == "__main__":
    ...
    # Resolve weights directory and display name from config
    ...
    # ``with_mtp`` here is the generator-level capability flag (controls verify
    # graph capture in ``from_pretrained``). Per-mode dispatch is decided at
    # ``generate()`` time via the ``mode.with_mtp`` kwarg. Interactive mode lets
    # users opt into MTP explicitly; benchmark mode keeps the generator MTP-
    # capable so the built-in w/ MTP benchmark rows can run.
    if args.interactive:
    ...
    if args.interactive:
        ...
        # Run selected benchmarks and collect stats + per-step data
        t0 = time.monotonic()
        workload_runners = []
        allowed_workloads = (
