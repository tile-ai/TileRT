...
from tilert.benchmark import merge_per_step, merge_stats, print_summary_table
...
if __name__ == "__main__":
    ...
    if args.interactive:
        ...
        all_per_step = [ps for _, ps in all_bench_results]
        ...
        if not args.no_plots:
            from tilert.benchmark.tps_curve import generate_tps_plots

            merged_ps = merge_per_step(all_per_step)
            pngs = generate_tps_plots(merged_ps, model_name, tag=args.tag)
            if pngs:
                print(f"\n## TPS-over-Position Curves ({model_name})\n")
                for png in pngs:
