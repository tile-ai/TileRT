...
# These base classes are shared by every model family (DeepSeek-V3.2, GLM-5,
# …). Each concrete model passes its OWN ``ModelArgs`` dataclass, so the base
# must not pin the parameter to one model's nominal type — annotate loosely
# while still defaulting to ``ModelArgs()`` at runtime.
ModelArgsLike = Any
...
class TileRTModule(nn.Module, ABC):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Annotate as model-agnostic so subclasses passing another model's
        # ModelArgs (e.g. GLM-5) aren't typed against DeepSeek's nominal type.
