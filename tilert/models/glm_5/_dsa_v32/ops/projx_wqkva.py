...
Standalone FP8 GEMV kernel for q, kv, pe_cache projections using the
generic GemvQMMA template.
"""
...
def projx_wqkva(
    ...
    *,
    ...
    Uses the generic GemvQMMA template. Supports both GLM5 (dim=6144)
    and DSV3.2 (dim=7168) via runtime dispatch based on x_quant.size(-1).
    """
...
class ProjXWqkva(TileRTModule):
    ...
    Uses GemvQMMA template instead of custom GemvMMA.
    Input is pre-quantized FP8 (from rmsnorm_quant), not raw bf16.
    """
    ...
    def golden_forward(
        ...
        if self.algorithm == ProjXWqkvaAlgorithm.FP16MMA:
            # FP16 input — direct float matmul
            x_float = x_quant.float()
        else:
            # FP8 input — dequant first
