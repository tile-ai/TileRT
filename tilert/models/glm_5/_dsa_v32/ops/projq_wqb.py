...
class ProjqWqbWeightsConverter(TilertWeightsConverter):
    ...
    def convert_to_fp16mma(self, weights: list[torch.Tensor]) -> torch.Tensor:
        ...
        with torch.inference_mode():
            # 1. Get general-format weights first
            wkv_b_a, wkv_b_a_scales = self.convert_to_general(weights)
            # wkv_b_a: [n_heads, kv_lora_rank, head_dim] fp8
            # wkv_b_a_scales: [n_heads, kv_lora_rank//128, head_dim//impl_block_size]
            ...
            # 3. Process scales per CTA
            kScalesPerPage = head_dim // 64  # scale_k_block=64
            ...
            # 4. Pack per CTA, 128-byte aligned
    ...
    def convert_to_bf16mma(self, weights: list[torch.Tensor]) -> torch.Tensor:
        ...
        with torch.inference_mode():
            ...
            # Compute n_local_heads (same logic as convert_to_general)
            ...
            # Input from device_sharding is already nope-extracted:
            #   weights: [n_local_heads, nope_head_dim, kv_lora_rank] FP8
            #   scales:  [n_local_heads, nope_head_dim // hd_block, kv_lora_rank // 128]
            # where hd_block = self.head_dim_block_size (may be < 128 for GLM5).
            # Dequantize using raw scales with per-block broadcast (handles non-128 M block).
            ...
            # 3. Reshape + swizzle (same as FP16MMA)
            ...
            mat_bytes = rows_per_cta * head_dim * 2  # bf16 = 2 bytes
...
class ProjqWqb(TileRTModule):
    ...
    def init_tilert_weights_hmma(self, state_dict: dict[str, torch.Tensor]) -> None:
        ...
        # A 1-element tensor keeps get_weights_list() and C++ SetInst happy.
