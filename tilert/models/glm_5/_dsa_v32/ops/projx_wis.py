...
class ProjxWis(TileRTModule):
    ...
    # HMMA config per model: (n_ctas, rows_per_cta, num_pages)
    _HMMA_CONFIGS = {
        7168: (4, 16, 7),  # DSV3.2: kOutputDim=64, 4 CTAs
        6144: (2, 16, 6),  # GLM5:   kOutputDim=32, 2 CTAs
    }
    ...
    def _swizzle_mma_16x16(mat_in: torch.Tensor) -> torch.Tensor:
        """Swizzle each 16x16 BF16 tile for MMA m16n8k16 loading."""
    ...
    def _to_hmma_layout(
        ...
        """Convert [output_dim, x_dim] BF16 weights to HMMA layout.

        Returns: [n_ctas, num_pages * rows_per_cta * cols_per_page] BF16 (swizzled, contiguous)
        """
        cols_per_page = x_dim // num_pages
        n_k_tiles = cols_per_page // 16
        # [M, K] -> [n_ctas, rows_per_cta, num_pages, cols_per_page]
        w = w_orig.reshape(n_ctas, rows_per_cta, num_pages, cols_per_page)
        # -> [n_ctas, num_pages, rows_per_cta, cols_per_page]
        w = w.transpose(1, 2)
        # Split into 16x16 MMA tiles: [n_ctas, num_pages, row_tiles, k_tiles, 16, 16]
