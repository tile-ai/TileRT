...
def _mark_weights_initialized(module: TileRTModule) -> None:
    ...
    Most leaf ops don't set is_tilert_weights_init=True in their init_tilert_weights.
    We need to set it explicitly so the skip logic in SerializableTileRTModule works.
    """
...
def _extract_ffn_ops(dsa: "Dsa") -> list:
    ...
    These ops hold already-converted GPU tensors and can be injected into a new Dsa.
    All extracted ops are recursively marked as initialized so init_tilert_weights
    skips them when the new Dsa is constructed.
    """
...
def _get_moe_weight_keys(dsa: "Dsa") -> set[str]:
    ...
    Returns MOE keys minus MLA keys, so shared keys (e.g. x_rmsnorm_gamma)
    are NOT skipped — V2's MLA also needs them.
    """
    from tilert.models.glm_5._dsa_v32.modules.mlp import MlpBlock
    from tilert.models.glm_5._dsa_v32.modules.moe import MoeBlock
...
class ShowHandsDSALayer:
    ...
    def load_device_weights(
        ...
        for weight_file in target_files:
            ...
            if skip_keys:
                # Selective loading: only load tensors we actually need,
                # avoiding OOM from files containing both MOE and MLA keys.
                logger.info(
    ...
    def _init_weights(
        ...
        """
        self._v2_p2p: dict = {}  # Thread-safe: each thread writes to a unique key
        ...
        def __load_weights(device_id: int, model_path: str | None) -> None:
            ...
            with torch.cuda.device(device_id):
                ...
                # Save P2P tensors for cross-device pointer exchange.
                if device_id == 0:
        ...
        # Exchange P2P pointers for v2: fill GPU 0's peer_bufs with peer buffer
        # addresses from GPUs 1-7.
        if self._v2_p2p:
            gpu0 = self._v2_p2p[0]
            peer_bufs_cpu = torch.zeros(self.num_devices - 1, dtype=torch.int64)
            for i in range(self.num_devices - 1):
                dev_id = i + 1  # GPU 1 through 7
                # Point to ll_buf on each peer GPU (not idx_selects)
                peer_bufs_cpu[i] = self._v2_p2p[dev_id]["ll_buf"].data_ptr()
            # Bulk copy from CPU to GPU 0 (more efficient than element-wise)
    ...
    def get_top_n_logprobs(self, device_id: int = 0) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        The kernel writes top-256 sorted (log_prob, token_id) pairs per
        sequence position when logprobs export is enabled.
    ...
    def get_token_logprob(self, device_id: int = 0) -> torch.Tensor:
        ...
        When logprobs export is enabled, TOP_P_SCORES contains log(prob)
        of the sampled token instead of the Gumbel-perturbed score.
    ...
    def set_logprobs_enabled(self, enabled: bool) -> None:
        ...
        Updates the device flag tensor read by the kernel at runtime.
        No CUDA graph recapture needed — only the tensor value changes.
