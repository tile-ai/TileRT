...
class Dsa(SerializableTileRTModule):
    ...
    def __init__(
        ...
        # Select MLA variant for v2 pipeline
        ...
        # Create shared P2P tensors on the correct CUDA device.
        # All layers on the same device share these tensors; the end2end
        # module fills them with actual GPU pointers after all devices init.
        dev = f"cuda:{device_id}"
        n_peers = num_devices - 1  # 7 peer GPUs for Broadcast
        if device_id == 0:
            ...
            topk = model_args.index_topk  # 2048
            self.v2_ll_buf = torch.zeros(max_seq_len * topk * 2, dtype=torch.int32, device=dev)
            mla_kwargs = {"ll_buf": self.v2_ll_buf}

        # V2 PureMla (GPU 1-7) splits heads across (num_devices - 1) GPUs, not all 8.
        # MoE/Mlp keep full num_devices for expert sharding.
    ...
    def get_temp_vars(
        ...
        # V2 PureMla (GPU 1-7) splits n_heads across (num_devices - 1) GPUs
        # with ceil padding; GPU 0 (SparseSelectMla) doesn't use head-split
        # temp vars, so the padded count is safe for all devices.
        if self.device_id != 0:
