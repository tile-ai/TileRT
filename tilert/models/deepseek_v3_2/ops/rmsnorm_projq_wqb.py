...
class RmsnormProjqWqbWeightsConverter(TilertWeightsConverter):
    ...
    Supports TP configurations where n_heads is not evenly divisible by
    num_devices. In that case, n_local_heads is padded to meet the BF16
    GEMV kernel alignment requirement (n_local_heads * qk_head_dim must
    be divisible by kBf16NumCtas * kGemvPageSize = 640).

    Padded head weight rows are zero-filled.
    """

    # BF16 kernel CTA count — used for head padding alignment
    kBf16NumCtas = 80
    # GEMV page size (output rows per page)
    kGemvPageSize = 8
    ...
    def __init__(self, model_args: ModelArgs, num_devices: int):
        ...
        # Compute n_local_heads: ceil division + kernel alignment padding
    ...
    def _compute_n_local_heads(cls, n_total_heads: int, num_devices: int, qk_head_dim: int) -> int:
        """Compute padded n_local_heads that satisfies kernel alignment.

        When n_total_heads is evenly divisible by num_devices, returns
        n_total_heads // num_devices directly (no padding).

        Otherwise, rounds up ceil(n_total_heads / num_devices) to the
        smallest value where n_local_heads * qk_head_dim is divisible by
        kBf16NumCtas * kGemvPageSize (640).
        """
        ...
        # Find minimum n_local_heads >= base such that
        # n_local_heads * qk_head_dim is a multiple of align_unit
    ...
    def _redistribute_heads(
        ...
        Uses row-level contiguous slicing instead of per-head reshape,
        because qk_head_dim (192) may not be divisible by block_size (128),
        making per-head scale reshape impossible.

        Prerequisite: n_local_heads * qk_head_dim must be divisible by
        block_size (guaranteed by _compute_n_local_heads alignment).
    ...
    def _common_to_tilert_fp16mma(
        ...
        """Convert common weights to TileRT FP16 MMA (HMMA) layout.

        Weight packing: [num_ctas, 2_pages, mat_data + scales + padding]
        - nope CTAs first, then pe CTAs
        - Each CTA handles 32 output rows
        - FP8 weights swizzled for 16x16 MMA tiles
        - Scales: float32 viewed as fp8 bytes, padded to 128 bytes
        """
        pages = 2
        rows_per_cta = 32

        # Determine CTA counts
    ...
    def convert_to_bf16mma(
        ...
        """BF16 MMA reuses the FP16 MMA packed layout (FP8 weights + scales).

        The page is byte-identical between FP16 and BF16 MMA paths; only the
        in-kernel dequant + MMA precision differ.
        """
...
class RmsnormProjqWqb(TileRTModule):
    ...
    Device Group B variant: no iq output, H=20 per GPU with padding.
    """
    ...
    def __init__(
        ...
        # Compute n_local_heads dynamically — pads when n_heads % num_devices != 0
        ...
        self.qk_head_dim = self.qk_nope_head_dim + self.qk_rope_head_dim
        self.qk_local_dim = self.qk_head_dim * self.n_local_heads  # 20 * 192 = 3840
    ...
    def init_reference_weights(self, state_dict: dict[str, torch.Tensor]) -> None:
        ...
        Dequantizes the full FP8 weight first, then slices this device's
        heads (with zero-padding for non-existent heads). This avoids the
        per-head scale reshape issue (qk_head_dim=192 not divisible by block_size=128).
        """
        ...
        # Dequantize full weight to bf16
        wq_b_bf16_full = weight_dequant(wq_b_full, wq_b_scale_full)  # [N_total*qk_head_dim, K]

        # Slice this device's rows (with padding for extra heads)
    ...
    def init_random_weights(self) -> None:
        ...
        Generates weights directly in per-device format (H=20, 3840 rows)
        to avoid the scale-redistribution problem (qk_head_dim=192 is not
        divisible by block_size=128, so FP8 scales cross head boundaries).
        """
        q_norm = torch.randn(self.q_lora_rank, dtype=torch.float32)

        # Per-device weights: fp8 with matching scale dtype
