...
FP8 MMA GEMV for q + ki projections with configurable unroll factor.
"""
...
def projx_wqaki(
    ...
    *,
    ...
    """FP8 MMA projection for q, ki with unrolled GEMV.
...
class ProjxWqakiWeightsConverter:
    ...
    def convert_dsv32(
        ...
        """Convert DSV3.2 weights to FP8 MMA page layout.

        104 CTAs, M=16, K=7168, 4 pages (2048,2048,2048,1024), bf16 scales, U=2.
        """
    ...
    def convert_glm5_68cta(
        ...
        """Convert GLM5 weights to FP8 MMA page layout (68CTA).

        68 CTAs, M=32, K=6144, 6 pages x 1024, float32 scales, U=2.
        """
        with torch.inference_mode():
    ...
    def convert_glm5_136cta(
        ...
        """Convert GLM5 weights to FP8 MMA page layout (136CTA).

        136 CTAs, M=16, K=6144, 3 pages x 2048, float32 scales, U=2.
        """
        with torch.inference_mode():
            wq_a_scale = wq_a_scale.to(torch.float32)
            wki_scale = wki_scale.to(torch.float32)
            ...
            ki_rows = 128
            total_rows = q_rows + ki_rows
            n_blocks = total_rows // 16  # 136
            scale_dim = dim // 128  # 48

            n_q_blocks = q_rows // 16  # 128
            n_ki_blocks = ki_rows // 16  # 8
            ...
            # 3 uniform pages of 2048 (6144/2048 = 3)
