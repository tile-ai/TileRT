...
Device Group A sparse selector entry: decoupled RMSNorm + FP8 quantize,
then ProjXWqaki (q + ki) + ProjXWis (idx_scores).
"""
...
class RMSNormProjxWqakisWeightsConverter(TilertWeightsConverter):
    ...
    def convert_to_decoupled(
        ...
        Packs wq_a + wki into FP8 MMA page layout for projx_wqaki kernel.
        Returns wis as a separate BF16 tensor for projx_wis kernel.
        ...
        # wis stays as BF16 for projx_wis kernel (no lossy fp8 truncation)
...
class RMSNormProjxWqakisRefWeightsAlias:
    ...
    Note: GLM5 HF model stores importance scores as BF16 `weights_proj.weight`
    (no FP8 quantization, no scale_inv). The dummy wis_scales is created in
    device_sharding() to keep the downstream converter compatible.
    """
...
class RMSNormProjxWqakis(TileRTModule):
    ...
    Runs three separate ops: RmsnormQuant → ProjXWqaki → ProjXWis.
    """
    ...
    def __init__(
        ...
        self.idx_head_dim = self.model_args.index_head_dim  # 128 (ki)
        self.idx_score_dim = self.model_args.index_n_heads  # 64 (DSV3.2) or 32 (GLM5)
        ...
        # GLM5 uses 68CTA pipe config (32 rows/block, 6 pages × 1024)
    ...
    def device_sharding(self, weights_map: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        GLM5 HF model stores wis (importance scores) as BF16 weights_proj.weight
        without FP8 quantization. A dummy all-ones scale is created so the
        downstream converter (which expects 7 tensors) works unchanged.
        """
        ...
        # wis is BF16 in GLM5 HF model (no FP8 quantization, no scale_inv)
        wis_weight = weights_map[self.ref_weights_alias.wis_weights][None, ...].repeat(
            self.num_devices, 1, 1
        )
        # Dummy all-ones scale so converter (which expects weight * scale) is a no-op
    ...
    def init_reference_weights(self, state_dict: dict[str, torch.Tensor]) -> None:
        ...
        # wis is BF16 in GLM5 HF model — no dequant needed (no scale_inv exists)
    ...
    def init_random_weights(self) -> None:
        ...
        tensor_list = [
            ...
            # wis is BF16 (like GLM5 HF model weights_proj.weight)
    ...
    def tilert_forward(
        ...
        # Step 2: projx_wqaki (FP8 MMA GEMV for q + ki only)
        projx_wqaki(
            self.x_rmsnorm_quant_out,
            self.x_rmsnorm_quant_scale_out,
            self.tilert_wqakis,
        ...
        # Step 3: projx_wis (BF16 GEMV for idx_scores, no lossy fp8 truncation)
