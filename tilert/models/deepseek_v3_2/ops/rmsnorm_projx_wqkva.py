...
Device Group B MLA entry: fused RMSNorm + 3-way GEMV
producing q, kv_cache, and pe_cache. Uses DECOUPLED algorithm:
rmsnorm_quant + standalone projx_wqkva.
"""
...
class RMSNormProjQKVAFP8MMAWeightsConverter:
    """Weight converter: pack FP8 weights into WqkvaPagedShared layout for GLM5 FP8 MMA kernel."""

    # Constants for GLM5 FP8 MMA weight packing
    HIDDEN_DIM = 6144
    Q_LORA_RANK = 2048
    KV_LORA_RANK = 512
    QK_ROPE_HEAD_DIM = 64
    TOTAL_ROWS = Q_LORA_RANK + KV_LORA_RANK + QK_ROPE_HEAD_DIM  # 2624
    ROWS_PER_CTA = 32
    NUM_CTAS = TOTAL_ROWS // ROWS_PER_CTA  # 82
    COLS_PER_PAGE = 1024
    NUM_PAGES = HIDDEN_DIM // COLS_PER_PAGE  # 6
    SCALES_PER_PAGE = COLS_PER_PAGE // 128  # 8
    BLOCK_SIZE = 128

    # sizeof(WqkvaPagedShared): mat[32*1024] + alignas(128) scales[8] + padding
    MAT_BYTES = ROWS_PER_CTA * COLS_PER_PAGE  # 32768
    SCALE_OFFSET = MAT_BYTES  # 32768 (already 128-aligned)
    PAGE_BYTES = ((MAT_BYTES + 128 + 127) // 128) * 128  # 32896

    @staticmethod
    def _swizzle_mma_16x32(mat_in: torch.Tensor) -> torch.Tensor:
        """Swizzle [*, 16, 32] tiles into MMA fragment layout."""
    ...
    def convert_to_fp8_mma_gemv(
        ...
        *,
        ...
        """Pack FP8 weights into WqkvaPagedShared layout for FP8 MMA kernel.

        Re-quantizes from per-(row,col)-block scales to per-column-block scales
        shared across all rows, matching the kernel's dequantization scheme.

        Supports both GLM5 (dim=6144, q_lora_rank=2048) and DSV3.2
        (dim=7168, q_lora_rank=1536) via explicit parameters.

        Args:
            hidden_dim: Model hidden dimension (6144 for GLM5, 7168 for DSV3.2).
            q_lora_rank: Q projection rank (2048 for GLM5, 1536 for DSV3.2).
        """
        ...
        # Validate tensor shapes match declared dimensions
        ...
        # Step 1: Dequant all weights to float using per-block scales
        ...
        w_float = torch.cat([wq_a_f, wkv_a_f, w_pe_f], dim=0)  # [total_rows, hidden_dim]

        # Step 2: Re-quantize with per-column-block scales (shared across rows)
        w_blocks = w_float.reshape(total_rows, hidden_dim // block_size, block_size)
        col_max = w_blocks.abs().amax(dim=(0, 2))  # [hidden_dim // block_size]
        ...
        # Pack into WqkvaPagedShared layout (vectorized, no Python loops)
        ...
        # Reshape into CTA × page structure
        mat = w_bytes.reshape(num_ctas, C.ROWS_PER_CTA, num_pages, C.COLS_PER_PAGE)
        mat = mat.transpose(1, 2)

        # Split into 2 M-blocks of 16 rows, tile into [16, 32] blocks, transpose for swizzle
        ...
        # Padding to PAGE_BYTES alignment
        ...
        # Assemble pages: [mat_data | scales | padding] per page
...
class RMSNormProjQKVAFP16MMAWeightsConverter:
    """Weight converter: pack FP16 weights for GemvFP16MMA kernel."""

    KV_LORA_RANK = 512
    QK_ROPE_HEAD_DIM = 64
    ROWS_PER_CTA = 32
    COLS_PER_PAGE = 512  # Half of FP8's 1024 (FP16 is 2x bytes per element)
    BLOCK_SIZE = 128

    @staticmethod
    def _swizzle_mma_16x16(mat_in: torch.Tensor) -> torch.Tensor:
        """Swizzle [*, 16, 16] tiles into m16n8k16 MMA fragment layout."""
    ...
    def convert_to_fp16_mma_gemv(
        ...
        *,
        ...
        """Pack weights into FP16 swizzled layout for GemvFP16MMA kernel.

        Dequants from FP8+scales to float, converts to FP16, swizzles for
        m16n8k16 MMA, packs into pages. No per-column scales needed.
        """
        ...
        num_k_tiles = cols_per_page // 16  # 32 tiles of 16 along K

        # Step 1: Dequant all weights to float
        wq_a_f = weight_dequant(wq_a.reshape(q_lora_rank, hidden_dim), wq_a_scale)
        wkv_a_f = weight_dequant(wkv_a.reshape(kv_lora_rank, hidden_dim), wkv_a_scale)
        w_pe_f = weight_dequant(w_pe.reshape(qk_rope_head_dim, hidden_dim), w_pe_scale)
        ...
        # Step 3: Reshape into CTA x page structure
        # [total_rows, hidden_dim] -> [num_ctas, rows_per_cta, num_pages, cols_per_page]
        mat = w_fp16.reshape(num_ctas, rows_per_cta, num_pages, cols_per_page)
        mat = mat.transpose(1, 2)  # [num_ctas, num_pages, rows_per_cta, cols_per_page]

        # Step 4: Split into 2 M-blocks of 16, tile into [16, 16], swizzle
        mat = mat.reshape(num_ctas, num_pages, 2, 16, num_k_tiles, 16)
        mat = mat.transpose(3, 4)  # [num_ctas, num_pages, 2, num_k_tiles, 16, 16]
        mat = C._swizzle_mma_16x16(mat)
        mat = mat.contiguous()

        # Step 5: Pack into byte stream
        # mat_bytes per page = rows_per_cta * cols_per_page * 2 = 32768
...
class RMSNormProjxWqkvaRefWeightsAlias:
    ...
    Note: In GLM5 (and DSV3.2), the PE projection is embedded in the last
    ``qk_rope_head_dim`` rows of ``kv_a_proj_with_mqa.weight``.  There is no
    separate ``self_attn.w_pe.weight`` in the HF checkpoint.  The split into
    kv_a and w_pe happens in ``device_sharding()``.
    """
...
class RMSNormProjxWqkva(TileRTModule):
    ...
    Uses DECOUPLED algorithm: rmsnorm_quant + standalone projx_wqkva,
    avoiding shared memory overflow in piped pipelines at seq_len >= 2.
    """
    ...
    def __init__(
        ...
        self.dim = self.model_args.dim  # 7168 or 6144
        self.q_lora_rank = self.model_args.q_lora_rank  # 1536 or 2048
        self.kv_lora_rank = self.model_args.kv_lora_rank  # 512
        self.qk_rope_head_dim = self.model_args.qk_rope_head_dim  # 64
        ...
        # Legacy scale tensor (unused in bf16 native, but needed for torch binding)
        self.tilert_wqkva_scales = torch.zeros((1, 1), dtype=torch.bfloat16)

        # DECOUPLED intermediate buffers
    ...
    def device_sharding(self, weights_map: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        The HF ``kv_a_proj_with_mqa.weight`` contains both kv_a (first
        ``kv_lora_rank`` rows) and w_pe (remaining ``qk_rope_head_dim`` rows).
        This method splits them and outputs 7 tilert tensors.
        """
        ...
        # Split combined kv_a_proj_with_mqa into kv_a and w_pe
        ...
        # Split scale the same way (each scale row covers block_size weight rows)
    ...
    def init_random_weights(self) -> None:
        ...
        tensor_list = [
            ...
            # Combined kv_a_proj_with_mqa: [kv_lora_rank + qk_rope_head_dim, dim]
    ...
    def tilert_forward(
        ...
        # Lazy imports to avoid circular dependency with projx_wqkva.py
        ...
        # Step 2: Standalone FP8 MMA GEMV for q, kv, pe_cache
        _projx_wqkva(
            self.x_rmsnorm_quant_out,
            self.x_rmsnorm_quant_scale_out,
            self.tilert_wqkva,
        ...
        # Extract pe at cur_pos for comparison
        seq_len = x.size(-2)
