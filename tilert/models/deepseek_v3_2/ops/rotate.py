...
Single rotate_op dispatches by runtime n_heads in C++.
"""
...
def rotate(
    ...
    Unified for deepseek_v3_2 (64 heads) and glm_5 (32 heads).
    Single rotate_op dispatches by runtime n_heads in C++.
    ...
    Args:
        ...
        kv_cache: Optional post-Hadamard cache write target.
        cur_pos: Optional [1] int32 tensor. Required when kv_cache is set.
        cache_base: Base row index added in compressed mode.
        cache_stride: Divisor (compressed) or window size (window). Must be
            > 0 when ``cache_compressed=True``.
        cache_compressed: When True, writes a single ratio-window slot at
            ``cache_base + cur_pos // cache_stride``; requires ``s == 1``.
            When False, window ring.
