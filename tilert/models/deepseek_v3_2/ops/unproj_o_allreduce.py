...
class UnProjOAllReduceWeightsConverter(TilertWeightsConverter):
    ...
    def convert_to_fp16mma_128cta(
        ...
        """Convert weights to FP16 MMA layout for 128-CTA config (GLM5 or DSV3.2).

        GLM5: dim=6144, dim_per_sm=48, 3 full tiles (16 rows each).
        DSV3.2: dim=7168, dim_per_sm=56, 3 full tiles + 1 remainder (8 rows).
        """
        with torch.inference_mode():
            ...
            block_size = self.model_args.block_size  # 128
            sms = 128
            vec_dim = mat.shape[-1]  # 2560
            dim_per_sm = dim // sms
            full_tiles = dim_per_sm // 16
            remainder_rows = dim_per_sm % 16
            stages = vec_dim // 512
            vec_scale_dim = vec_dim // block_size
            scale_per_stage = vec_scale_dim // stages  # 4

            # --- Scales: build per-CTA, per-tile, per-stage layout ---
            # GLM5 (dim_per_sm=48): 48 < 128, so each CTA's rows always fall
            #   within one scale block. Use repeat(1, 8, 1) → 1 scale per tile.
            #   Layout: [num_tiles * scale_per_stage] = [3 * 4] = 12 per page.
            #
            # DSV32 (dim_per_sm=56): tiles can cross 128-row scale block boundary.
            #   Use repeat(1, 16, 1) → 2 scales per full tile (lo/hi 8-row halves),
            #   1 scale per remainder tile. Layout per page:
            #   [full_tiles * 2 * kblocks + 1 * kblocks] = [3*2*4 + 1*4] = 28.
            dim_scale_dim = dim // block_size
            scales_per_full_tile = 2 if remainder_rows > 0 else 1
            # Total scales per page: full_tiles * scales_per_full_tile * kblocks
            #                        + (1 if remainder) * 1 * kblocks
            ...
            # sc shape: [dim_scale_dim * repeat_factor, vec_scale_dim]
            # = [sms * (full_tiles * scales_per_full_tile + rem_scales), vec_scale_dim]
            ...
            # --- Mat: reshape to per-SM, split full tiles + remainder ---
            mat_per_sm = mat.reshape(sms, dim_per_sm, vec_dim)

            # Full tiles: first full_tiles*16 rows
            ...
            if remainder_rows > 0:
                # Pad remainder (8 rows) to 16 rows, apply same 16x16 swizzle.
                # Kernel loads full uint2 (same as full tiles) but only uses
                # the first 8 output rows from the MMA result.
                mat_rem_raw = mat_per_sm[:, full_rows:, :]  # [sms, 8, vec_dim]
    ...
    def convert_to_bf16mma(
        ...
        Only the in-kernel dequant + MMA precision differ; the on-disk /
        in-memory packed page is byte-identical, so we delegate to the FP16
        converter.
        """
        assert (
    ...
    def convert_to_fp16mma(
        ...
        with torch.inference_mode():
            ...
            dim = self.model_args.dim  # 6144
            block_size = self.model_args.block_size  # 128
            sms = 128  # use 128 sms for glm_5
            vec_dim = mat.shape[-1]  # 2048 (v1 H=8) or 2560 (v2 H=10)
            dim_per_sm = dim // sms  # 48
            tiles_per_stage = dim_per_sm // 16  # 3
            stages = vec_dim // 512  # 4 (v1) or 5 (v2)
            dim_scale_dim = dim // block_size  # 48
            vec_scale_dim = vec_dim // block_size  # 16 or 20
            scale_per_stage = vec_scale_dim // stages  # 4
...
class UnProjOAllReduce(TileRTModule):
    ...
    def device_sharding(self, weights_map: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        if self.n_heads % self.num_devices == 0:
            ...
            # Padded column redistribution for uneven head split
            # Weight: [dim, n_heads * v_head_dim] → per-head columns
            ...
            # Scale: columns are aligned per-head (v_head_dim % block_size == 0)
