...
class ExpertSelectUpGateSiLUWeightsConverter(TilertWeightsConverter):
    ...
    def convert_to_mma(
        ...
        with torch.inference_mode():
            ...
            moe_rows = weights_w1.shape[1]  # moe_inter_dim / num_devices
            n_row_groups = moe_rows // 16  # e.g. 16 (8GPU) or 128 (1GPU)
            ...
            scales_per_page = 1024 // args.block_size  # 8
            repeat_factor = n_row_groups // scale_m_dim  # block_size / 16 = 8
            scales_w1 = (
    ...
    def convert_to_bf16mma(
        ...
        Only the in-kernel dequant + MMA precision differ; the page is
        byte-identical, so we delegate to the FP16 converter.
        """
        return self.convert_to_mma(weights_list, "fp16mma")
...
class ExpertSelectUpGateSiLU(TileRTModule):
    ...
    def __init__(
        ...
        # Skip CUDA alloc when no GPU is visible (offline conversion).
        self.tilert_scales = (
    ...
    def init_reference_weights(
        ...
        # bf16 cast: ref_* are only read by golden_forward, which re-casts to
        # float before matmul. The kernel under test is fp8, so bf16-quantized
        # reference is closer to true operator behavior than fp32. Cast per
        # tensor before stacking to avoid a full-size fp32 intermediate.
