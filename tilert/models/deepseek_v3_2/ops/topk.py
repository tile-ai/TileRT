...
def topk_accurate(
    ...
    Args:
        ...
        topk (int): The number of topk to approximate.
        profile_logs (torch.Tensor): The profile logs tensor.
        ratio (int): Token-domain → logits-trailing-dim compression factor.
            ratio=1 (DSv3.2 sparse_index): logits trailing dim is per-token.
            The kernel computes per-sample valid count as
            (cur_pos + sample_id) / ratio + 1.
    Returns:
        indices (torch.Tensor): The output tensor.
    """
    if logits.dtype != torch.float32:
    ...
    # here topk_accurate_op needs the cur_pos of the first sample
...
class TopK(nn.Module):
    ...
    def tilert_forward(
        ...
        # seq_len for last sample = cache_len; cur_pos = cache_len - num_samples
        ...
        # topk_accurate returns (num_samples, topk); add batch dim for (1, num_samples, topk)
        if indices.dim() == 2:
