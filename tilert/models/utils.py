...
# Sentinels distinguish "unset" from `None`: `factor_override=None` disables
# the YaRN smoothed-ramp branch; `theta_override=None` is rejected (theta has
# no off-switch).
...
def precompute_freqs_cis(  # type: ignore[no-untyped-def]
    ...
    *,
    ...
    Args:
        ...
        factor_override: If unset, ``args.rope_factor`` is used. Pass
            ``None`` to disable the YaRN smoothed-ramp branch even when
            ``args.rope_factor`` is set. Pass a numeric value to override the
            factor inline.
